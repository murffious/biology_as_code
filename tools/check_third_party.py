...
"""Every sizeable data file this PUBLIC repo ships must be accounted for.
  ...
  python3 tools/check_third_party.py            report
  python3 tools/check_third_party.py --strict   exit 1 on any FAIL
...
MANIFEST = ROOT / "THIRD-PARTY-DATA.json"
MIN_BYTES = 20_000
...
def main(argv: list[str]) -> int:
    ...
    for e in m["entries"]:
        if not e.get("blocks"):
            continue
        notice = ROOT / (Path(e["file"]).stem + ".NOTICE.md")
        if e["file"].endswith("/"):
            notice = ROOT / e["file"] / "NOTICE.md"
        if not notice.is_file():
            fails.append(f"{e['id']} restricts {e['blocks']} but has no {notice.name}")

    # 3. NOTICE must carry the third-party section.
    n = (ROOT / "NOTICE").read_text(encoding="utf-8") if (ROOT / "NOTICE").is_file() else ""
    ...
    c = (ROOT / "CITATION.cff").read_text(encoding="utf-8") if (ROOT / "CITATION.cff").is_file() else ""
