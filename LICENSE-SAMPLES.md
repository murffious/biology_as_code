# License — schemas & examples

> **Status note — under review, August 2026.** The project owner operates no
> commercial implementation of this specification. Product identifiers have been
> removed from `src/` and `tests/` and a CI gate now keeps them out (see
> `design/BASELINE.md`). **Disposition of the patent claims described below is a
> pending legal decision.** This document is retained unchanged pending that
> decision — it is not evidence of an active commercial program, and nothing
> here should be read as having been withdrawn. Paths named below that pointed
> into the package were updated where the code moved; the substantive terms are
> untouched.

Copyright (c) 2026 Biology as Code project contributors.

Permission is hereby granted, free of charge, to any person obtaining a copy of the
**schemas** and **examples** in this repository (the "Samples"), to use, copy, modify,
merge, publish, and distribute the Samples, subject to the following conditions:

1. Attribution: include a link to https://github.com/murffious/biology_as_code and
   the book title *Biology as Code: Standardizing Nutrition Science for Preventive Medicine*.
2. The Samples are provided "AS IS", without warranty of any kind.
3. This license does **not** grant rights to the full book manuscript, figures, or brand
   materials, which remain all rights reserved unless separately licensed.
4. This license does **not** grant rights to any **product meal score**, **Kibo-vars
   product scorer**, or other patent-pending scoring engines. Those are never part of
   the Samples and must not appear in this repository (see `PROPRIETARY_IP.md`).

Optional SPDX for sample files: `LicenseRef-BAC-Samples-Attribution`
