...
Nutrition fails when it **accounts** (calories, macros, slogans) instead of **executing** (packets through gates under host state).

This project standardizes the **interfaces**:

- typed food packets  
- gate vs bound  
- claim audit that stops when a gate is closed  

All three are executable: see [cookbook 4 — auditing a real claim](cookbook/04-claim-audit.md).

## For engineers

Build against the constitution:
...
Adding a teaching pathway graph? Use the
[pathway contribution template](python/ADD_PATHWAY.md)
(registry wire → export mermaid → integration check).
