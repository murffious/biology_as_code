"""Smoke + promotion tests for the unified kibo_core package."""
...
from biology_as_code.data.kibo_core.paths import PACKAGE_ROOT


class TestKiboCore(unittest.TestCase):
    def test_import_package(self):
        from biology_as_code.data import kibo_core

        self.assertEqual(len(kibo_core.SEVEN_SYSTEMS), 7)

    def test_registry_has_promoted_laws(self):
        from biology_as_code.data.kibo_core.laws import load_system_bound_registry
    ...
    def test_iron_walk(self):
        ...
        from biology_as_code.data.kibo_core.pathways import NONHAEM_IRON_PATHWAY
    ...
    def test_organ_bounds(self):
        from biology_as_code.data.kibo_core.geography import ORGAN_BOUNDS
    ...
    def test_promoted_shape_and_systems(self):
        for lid, expect in self.PROMOTED.items():
            with self.subTest(lid=lid):
                L = self.reg.get(lid)
                self.assertEqual(L.kibo_system, expect["system"])
...
def _load_topics_classifier():
    ...
    # .../biology_as_code/src/biology_as_code/data/kibo_core/tests/this_file
...
class TestTopicLawLinks(unittest.TestCase):
    ...
    def test_ontology_has_no_retired_stub_links(self):
        from biology_as_code.data.kibo_core.topics import load_topics
...
class TestL2RedoxCalciumCompetition(unittest.TestCase):
    def test_high_calcium_narrows_iron_factor(self):
        from biology_as_code.data.kibo_core.sim import MetabolicState
        from biology_as_code.data.kibo_core.sim.rules_redox import apply_l2_redox_competition
