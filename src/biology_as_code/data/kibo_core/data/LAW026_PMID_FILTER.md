...
**KIBO:** Prefer `fermentable_fraction` + matrix context over total fiber_g in FLOW; do not lock UNITS from this alone.
...
**KIBO:** Colon fermentation node is not “zero energy out”; product vector = acetate/propionate/butyrate.
...
**KIBO:** `rs_profile` + `MicrobiomeProfile` are first-class; magnitude_locked stays false.
...
**SSI (provisional):** 🟡.  
**KIBO:** Payload form fields matter for colon stage; keep FLOW demos provisional.
...
**KIBO:** Ancillary support only; prefer human/in vitro human-inocula packs for lock.
...
**SSI (provisional):** 🟡.  
**KIBO:** Aligns with `FiberProperties` (viscosity, fermentability); not a magnitude source.
...
**KIBO:** Optional catalog support for colonic medium substrate types.
